...
class IQLSparkSqlParser(conf: SQLConf) extends AbstractSqlParser {
  val astBuilder = new IQLSparkSqlAstBuilder(conf)
...
/**
  ...
  */
class IQLSparkSqlAstBuilder(conf: SQLConf) extends SparkSqlAstBuilder(conf) {
